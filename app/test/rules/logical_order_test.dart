...
void main() {
  ...
  test('the stress scenario: a stolen base noticed two pitches late lands '
    ...
    expect(
      ...
      reason:
          'the steal is spliced in right after pitch1, not appended '
  ...
  test('two inserts anchored to the same event tie-break by their own '
      'recording order', () {
    final b = EventBuilder();
    final anchor = _ball(b, 'anchor');
    ...
    expect(
      ...
      reason:
          'both anchor to the same event; order matches the order '
  ...
  test('an insert anchored to a since-voided event keeps its position', () {
    ...
    expect(
      ...
      ['before', 'insert', 'after'],
      reason:
          'anchor is hidden (voided), but insert still sits where '
          "anchor's raw position was — not wherever it was recorded",
    );
  });

  test('an insert anchored to a corrected (non-head) event resolves through '
    ...
    expect(visible.map((e) => e.id).toList(), [
      'before',
      'correction',
      'insert',
      'after',
