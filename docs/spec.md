...
The loop still records the strikeout immediately, because that is right for almost every third strike and keeps the book correct if the scorer moves on. Resolving **voids that out and writes what happened**, as one batch and one undo unit. The voided out is not noise: it is the record that the app called a strikeout and the scorer said the ball was uncaught, hidden from the visible stream by §6 and kept in the raw one.
