# Diamond — Event Taxonomy & Pitch Entry Spec (v0.53)

**Status:** Draft for review — v0.53 widens §11.1's **"record last pitch"** backfill to the **uncaught third strike**, and gives its canvas the bounce hinge. The offer existed since v0.39 but was armed only for a ball in play, which was narrower than the reason for it: the affordance is for a pitch whose location was lost because *the scorer's attention left the zone*, and on a D3K the batter is running, so it left. Nothing else arms it — every unlocated pitch would make the offer chrome — and the hinge comes along because the ordinary D3K is a ball in the dirt, which a frontal-only canvas cannot say. v0.52 gives `batterAction` **a writer and three values instead of five** (§4.1, §11.1): `bunt` | `slap` | `slash`. The old five each carried two axes — what she *intended* and whether she *offered* — and the second **derives from the outcome**, so storing it stored a derived fact that could contradict its own source. §11.1's sheet gains the modifier tier that writes it, and a **one-tap getaway consequence**: marking a wild pitch or passed ball with runners aboard offers *all runners up one*, which is the whole play the overwhelming majority of the time and should not cost a trip to the field. **One tap, not zero** — a runner on third often holds on a ball that only trickled away — with action-scoped undo as the correction rather than a confirmation dialog. Going to the field instead seeds the ball **loose**, since a wild pitch's physics is the absence of a touch and seeding the catcher would assert something that did not happen. v0.51 rebuilds §11.1's **outcome sheet on frequency, not on a guess**: five primaries at fixed positions — Ball, Called strike, Swinging, Foul, In play — then everything else. The location suggestion is **deleted**, not demoted: it could only ever return `ball` or `called_strike`, since location knows nothing about whether the batter swung, so its most confident case was its worst — a pitch *in* the zone is more often swung at than taken. `ball_intentional` joins the sheet (four intentional balls is a different story from four missed spots) and so does `strike_unspecified`, which protects the count where `unknown` surrenders it: it says *a strike happened and I missed which kind*, and the count still advances. **`no_pitch` is removed** (§4.1) — nothing ever wrote it, and it existed only as three exclusions. Full history: `docs/spec-history.md`.
...
Batter advance (from lineup projection), count reset, inning flip, pitcher's pitch count, forced advances on walks and HBP (v0.41: auto-generated **and auto-applied** — no confirmation; the forced chain is rulebook arithmetic with nothing to decide, and the realistic error, a mistapped outcome, is one confirm couldn't catch and undo does), courtesy-runner prompts when the pitcher/catcher reaches base (RuleSet-gated), and D3K arming (uncaught third strike with first base open / two outs → Diamond prompts the runner resolution instead of assuming the out). Undo is always one tap, top-level, unlimited depth (§6) — and **action-scoped** (v0.41): one tap voids the last *action*, meaning the last scorer-authored root event (a `PitchThrown`, a `CountCorrection`, a committed play) together with everything the loop auto-appended for it, so a bases-loaded walk reverses as one unit, not five.
