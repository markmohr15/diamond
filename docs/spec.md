...
The box-score projection charges an error to a misplay touch (`dropped`/`booted`/`bobbled`/`wild_throw`/`missed_catch`/`tag_missed` — v0.43 adds the last: a muffed tag is a fielding error, while `deflected` is never a candidate) when **(a)** `ordinaryEffort` resolves true, and **(b)** the misplay had a consequence: a batter reached who'd otherwise be out, any runner advanced (via `enabledByTouchId` links), or an at-bat was prolonged (dropped foul fly). A dropped liner recovered in time to record the out anyway → misplay logged, no error charged. Both facts survive: the official book stays clean and the SS's development report still shows the drop.

`ordinaryEffort` defaults (v0.43 adds the last): `booted`, `missed_catch`, `dropped` (on throws and routine flies), `tag_missed` → true — **recording one of these is itself the claim that she should have made it**, and anything not worth charging is not worth putting in the play-by-play at all (a runner who beats the tag on a great slide is simply safe, and nothing is recorded). `wild_throw` stays a judgment call rather than a default: a throw can sail and cost nothing, leaving the runner exactly where a good throw would have; `dropped` on a touch marked at an extreme location relative to the fielder's position, or on `deflected` chains → prompt the scorer. The override is a long-press on the misplay in the play summary strip — deliberately *post-hoc friendly*: the judgment can be made or changed after the dust settles, or the next half-inning, or that night, and every projection recomputes.
...
- **The sacrifice bunt cannot derive, so it is asked.** Nothing physical separates bunting to move the runner from bunting for a hit — the difference is entirely intent, which is why it is §13's second judgment flag after `ordinaryEffort`. It is stored on the batted ball (`BallInPlay.sacrifice`), and the field surface offers it exactly when the question is live: a bunt that moved a runner up.
...
- **Events:** `PitchThrown{swinging_strike}` → `RunnerAdvance{batter, 0→1, dropped_third_strike}` → `FielderTouch{2, wild_throw, OE: true}` → `RunnerAdvance{batter, 1→2, wild_throw, ←throw}` → `RunnerAdvance{R1, 1→3, wild_throw, ←throw}`
...
**The batter runs on contact.** The surface opens with the batter-runner already walked up to first (the forced cascade applied — runners never pass each other), because a ball in play means she's running. Presumed movement **renders in motion — a third of the way up the line** — never parked on the next base: the canvas shows runners running, and the difference between "she's going there" and "she got there" stays visible until the scorer answers. A resolved runner stands on her base. The presumption resolves, never lingers: a **caught first touch** voids it — the unattributed walk-up comes off and the batter is out in the air, putout to the catch; an **out marked at the base a runner's last leg reached** absorbs that leg into the out (no phantom advance ever commits); and a **misplay first touch claims the provisional reach** (§13.2: hit-vs-error lives on the first touch), re-attributing her leg to it so `reached_on_error` derives with zero extra taps.
...
- **The Safe pair (§13.2, §11.3):** `passed_ball` is physics, never a ruling — the chip emits `FielderTouch{2, missed_catch, ordinaryEffort: true}` anchored to the most recent pitch plus the advance linked via `enabledByTouchId`; `wild_pitch` is the advance alone. A second runner advancing in the same between-pitch window on the same passed ball links to the already-emitted touch — one PB, one touch, N advances.
