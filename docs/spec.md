# Diamond — Event Taxonomy & Pitch Entry Spec (v0.36)

**Status:** Draft for review — v0.36 ships the **batter silhouette** on the entry canvas as a location cue rather than keeping it behind a debug flag, and answers the objection that kept it off: she is derived from the zone profile, with her knee at `y = 0` and her armpit at `y = 1`, so she tracks the rect for every batter by construction instead of being drawn at one fixed height. She is the *only* occupied-side cue — the batter's-box fill and shading are dropped from both planes — renders in the frontal plane alone, and takes no pointer events. Also records that batter height becomes settable with the default profile chosen by sport and age level (§3.1). v0.35 makes `BounceCoord.depth` **optional** (§3.3), so "in the dirt, depth unknown" is a storable observation rather than a hole: the §11.1 hinge always yields `x`, the second placement that sets depth can be skipped, and absent is never 0 (0 means the ball hit the plate's front edge). Also corrects §11.4's top-down plane, where the quoted −1 ft…+5 ft depth range was never achievable alongside px-per-x parity with the frontal plane — sharing px-per-x and staying isotropic *derive* the range from the drawing rect (61.2″ = 5.10 ft at 12U), leaving only the anchor free, which splits it at −2.5 ft…+2.60 ft so the plate's whole pentagon plus catcher-side room fits below the seam. The 0–2 / 2–4 / 4 ft+ bands go with it, replaced by an unsigned foot ruler, and the drawn catcher goes too — there is no rulebook position for one, so any mark is invented. v0.34 promotes §11.4's azimuth 0 from an incidental camera parameter to a stated constraint (off-axis breaks the exact lateral registration the section requires and tests for) and opens Open Question #9 on whether the ground perspective should tilt per batter handedness — shelved, not needed for v1 or M1. Full history: `docs/spec-history.md`.
