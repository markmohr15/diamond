# Diamond — Event Taxonomy & Pitch Entry Spec (v0.54)

**Status:** Draft for review — v0.54 **bounds undo at the plate appearance** (§6, §11.3) and has it hand back the scorer's own entry. A pitch's call, location and outcome commit as one `PitchThrown`, so voiding it discarded two answers nobody wanted to redo; undo now repopulates the loop from the voided payload and lands on the location step, while everything the loop appended automatically stays voided. And depth stops: a plate appearance seals when the next one *begins* — the first type tap — and **stays** sealed, because a floor defined relative to the current plate appearance walks backwards to the first pitch of the game as you peel. Unlimited depth was the original promise and it was the wrong one: every hazard in editing history needs undo to cross a plate-appearance boundary, and nothing needs it to. Reaching further back is an explicit edit, not a repeated tap. It is also now the **only** undo: the field surface's own ↺ and ✕ move into the top bar's pair, which sits on every screen, so the button can no longer reach past the surface the scorer is looking at — it used to void the pitch under an open play and leave the draft anchored to a voided event. On the field it steps back one entry at a time and, with nothing entered, takes the play off screen and the pitch with it. Light/dark joins that bar as a two-state toggle. v0.53 widens §11.1's **"record last pitch"** backfill to the **uncaught third strike**, and gives its canvas the bounce hinge. The offer existed since v0.39 but was armed only for a ball in play, which was narrower than the reason for it: the affordance is for a pitch whose location was lost because *the scorer's attention left the zone*, and on a D3K the batter is running, so it left. Nothing else arms it — every unlocated pitch would make the offer chrome — and the hinge comes along because the ordinary D3K is a ball in the dirt, which a frontal-only canvas cannot say. v0.52 gives `batterAction` **a writer and three values instead of five** (§4.1, §11.1): `bunt` | `slap` | `slash`. The old five each carried two axes — what she *intended* and whether she *offered* — and the second **derives from the outcome**, so storing it stored a derived fact that could contradict its own source. §11.1's sheet gains the modifier tier that writes it, and a **one-tap getaway consequence**: marking a wild pitch or passed ball with runners aboard offers *all runners up one*, which is the whole play the overwhelming majority of the time and should not cost a trip to the field. **One tap, not zero** — a runner on third often holds on a ball that only trickled away — with action-scoped undo as the correction rather than a confirmation dialog. Going to the field instead seeds the ball **loose**, since a wild pitch's physics is the absence of a touch and seeding the catcher would assert something that did not happen. Full history: `docs/spec-history.md`.
