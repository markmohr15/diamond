...
**The M1 D3K prompt (v0.41)** resolves the armed case with four one-tap outcomes, an interim subset of the full runner resolution (§15) and encoded per play #5's conventions — the batter's advance always carries `dropped_third_strike`, and a D3K `FielderTouch` anchors to the pitch event: **Out (tag)** → `RunnerOut{atBase: 1, how: tag}`; **Out (throw)** → `RunnerOut{atBase: 1, how: strikeout_d3k_throw}`; **Safe (wild pitch)** → `RunnerAdvance{0→1, dropped_third_strike}` alone; **Safe (passed ball)** → `FielderTouch{2, dropped, OE: true}` then `RunnerAdvance{0→1, dropped_third_strike, ←touch}`. The Safe pair is §13 working as designed: a passed ball is a catcher misplay and is recorded as physics, a wild pitch is the absence of one (§13.2) — the scorer taps a button, never a ruling.
