# Diamond — Event Taxonomy & Pitch Entry Spec (v0.43)

**Status:** Draft for review — v0.43 rewrites **§15.1's entry grammar** from field testing DIA-008 (design review with Mark): the **trajectory comes first** and gates the canvas — the batted-ball type drives every later option; the ball's path is either **drawn in two taps** (first bounce → where it ended up, rendered as an accent streak with live distances) or **implied by the fielder drag** — drag the fielder to where she made the play, her release point becomes the assumed landing *and* the touch's own `location` (§4.2), and a trajectory-driven **what-happened popup** disambiguates (air: caught / dropped / missed it / picked it up; ground: fielded / booted / missed it — "missed it" records no touch at all, §13). Runner drags now resolve on **Safe/Out targets that appear at the approached base** (the GameChanger interaction, kept because it was right); **the batter runs on contact** — the surface opens with her walked up to first, forced cascade applied, and the presumption resolves mechanically (a caught first touch voids it and records the fly out; an out at a reached base absorbs the leg; a misplay first touch claims the reach per §13.2); every popup is a **centered modal sized to its content**, the trajectory question included; SAFE and OUT releases each get a **classified popup with disjoint vocabularies** (safe: the hit itself / on the throw / on an error / fielder's choice / obstruction; out: force/tag/fly-out + interference), with hit rank refined so only the hit itself raises it; **catcher's interference becomes an outcome variant** (§4.1 schema change, HBP's pattern) scored **E2 by rule** (§13.2); and the ⚖ rule calls (obstruction, runner interference) move off the standing strip onto the **runner-consequence sheets** — they are baserunning facts and appear only where baserunning is edited. v0.42 adds **§15.6, the idle field** (DIA-008d): a field affordance on the loop's idle surfaces opens the canvas with no ball in play — defense and runners visible (alignment drag remains §16.4, M2) — and between-pitch runner events enter by drag + reason chip, committing immediately with no chain and no journal: advances `stolen_base` (default) / `wild_pitch` / `passed_ball` / `defensive_indifference`, outs `caught_stealing` (default) / `picked_off`. `passed_ball` emits §13.2's physics pair — catcher-misplay touch anchored to the most recent pitch, advance linked via `enabledByTouchId`, later runners in the same window reusing the touch — and each entry is one action-scoped undo unit that never rides the preceding pitch's. v0.41 removes the confirmation from walk/HBP forced advances (§11.3): the chain is rulebook arithmetic with nothing to decide, so it auto-applies, and undo becomes **action-scoped** — one tap voids the last scorer action *and* everything the loop auto-appended for it, so a bases-loaded walk reverses as one unit. D3K arming is redefined to **key on the catch, not the pitch** — uncaught = the pitch bounced *or* a catcher-misplay touch on any third strike, called or swinging — and the resolution flow (four one-tap fast paths plus play #5's full chain) moves to DIA-008, where the misplay entry surface lives; until then the loop records the K out and one action-scoped undo reverses it. §13.2 gains the **WP/PB derivation rule** the earned-run reconstruction was already assuming: a passed ball is an ordinary-effort catcher misplay touch against the pitch, a wild pitch is an uncaught pitch with no such touch — derived from physics, never stored. §4.1 adds **`strike_unspecified`** (schema change), bailout's word for "the count advanced and I don't know how" — full count effect, excluded from swing analytics — and §11.2's bailout row becomes BALL / STRIKE / FOUL / IN-PLAY, with the two-strike FOUL-vs-STRIKE read settled by whether the at-bat ended rather than by any judgment about the pitch. Full history: `docs/spec-history.md`.
...
**D3K arming keys on the catch, not the pitch (v0.41).** An uncaught third strike is either `swinging_strike_blocked` — the pitch bounced — or *any* third strike, called or swinging, carrying a catcher-misplay touch (`FielderTouch{2, dropped}`, §13.2): the ball in the dirt and the clean pitch dropped are equally live, and only the first is visible on the pitch itself. Because the misplay-touch entry surface is §15's play chain (DIA-008), the D3K resolution flow lands there — play #5 enterable end-to-end, plus one-tap fast paths for the plays that end at first: **Out (tag)** → `RunnerOut{atBase: 1, how: tag}`; **Out (throw)** → `RunnerOut{atBase: 1, how: strikeout_d3k_throw}`; **Safe (wild pitch)** → `RunnerAdvance{0→1, dropped_third_strike}` alone; **Safe (passed ball)** → `FielderTouch{2, dropped, OE: true}` anchored to the pitch, then the same advance linked via `enabledByTouchId`. The Safe pair is §13 working as designed — a passed ball is recorded as physics, a wild pitch is the absence of one, never a ruling. Until DIA-008 lands, the loop records the strike-three out unconditionally, and a real D3K is reversed with one action-scoped undo: the pitch and its out, one unit.
...
**Wild pitch vs. passed ball (v0.41)** is the same derivation pattern applied to an uncaught pitch: a **passed ball** is an ordinary-effort catcher misplay touch (`dropped`/`missed_catch`, `ordinaryEffort: true`) recorded against the pitch; a **wild pitch** is an uncaught pitch with no such touch — the ball getting away was the pitcher's doing (bounced, sailed), which the pitch's own record (`bounceLocation`, `swinging_strike_blocked`) usually already shows. Both are derived at projection time from physics, never stored as rulings, and both feed §13.3's earned-run reconstruction, which replays innings "as if errors and passed balls hadn't happened." This is what the D3K prompt's Safe pair (§11.3) writes, and what runner advances on `wild_pitch`/`passed_ball` reasons (§4.3) are reconciled against.
...
The field canvas is not only a batted-ball surface. A field affordance on the call screen (and every idle loop surface) opens the same canvas with **no ball in play**: defense at their current spots, runners at their bases, count HUD still up, no chain strip. Two jobs live here:
...
- **Between-pitch runner events.** Drag a runner to a base → `RunnerAdvance`, reason from a chip row: `stolen_base` (default, pre-selected), `wild_pitch`, `passed_ball`, `defensive_indifference`. Drag a runner to the out affordance at a base → `RunnerOut`: `caught_stealing` (default), `picked_off`. The chip tap commits immediately — one event (or one pair, below), no ✓, no journal — because each entry is a single fact, not an accumulating chain; §15.5's commit machinery is for plays.

Two consistency rules carry over from the rest of the spec:

- **The Safe pair (§13.2, §11.3):** `passed_ball` is physics, never a ruling — the chip emits `FielderTouch{2, dropped, ordinaryEffort: true}` anchored to the most recent pitch plus the advance linked via `enabledByTouchId`; `wild_pitch` is the advance alone. A second runner advancing in the same between-pitch window on the same passed ball links to the already-emitted touch — one PB, one touch, N advances.
