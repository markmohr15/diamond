# Diamond — Event Taxonomy & Pitch Entry Spec (v0.52)

**Status:** Draft for review — v0.52 gives `batterAction` **a writer and three values instead of five** (§4.1, §11.1): `bunt` | `slap` | `slash`. The old five each carried two axes — what she *intended* and whether she *offered* — and the second **derives from the outcome**, so storing it stored a derived fact that could contradict its own source. §11.1's sheet gains the modifier tier that writes it, and a **one-tap getaway consequence**: marking a wild pitch or passed ball with runners aboard offers *all runners up one*, which is the whole play the overwhelming majority of the time and should not cost a trip to the field. **One tap, not zero** — a runner on third often holds on a ball that only trickled away — with action-scoped undo as the correction rather than a confirmation dialog. Going to the field instead seeds the ball **loose**, since a wild pitch's physics is the absence of a touch and seeding the catcher would assert something that did not happen. v0.51 rebuilds §11.1's **outcome sheet on frequency, not on a guess**: five primaries at fixed positions — Ball, Called strike, Swinging, Foul, In play — then everything else. The location suggestion is **deleted**, not demoted: it could only ever return `ball` or `called_strike`, since location knows nothing about whether the batter swung, so its most confident case was its worst — a pitch *in* the zone is more often swung at than taken. `ball_intentional` joins the sheet (four intentional balls is a different story from four missed spots) and so does `strike_unspecified`, which protects the count where `unknown` surrenders it: it says *a strike happened and I missed which kind*, and the count still advances. **`no_pitch` is removed** (§4.1) — nothing ever wrote it, and it existed only as three exclusions. v0.50 gives `RunnerAdvance` a **`cause`** field (§13.2, §4.3), because `reason` had been answering two questions at once — *why the runner was entitled to move* and *what happened to the ball*. They coincide on a runner's advance and come apart on a **dropped third strike**, where entitlement must stay `dropped_third_strike` for the batting line while the ball is separately a wild pitch, a passed ball, or neither. `cause` holds the second, and its absence is an answer rather than silence: she reached on an error, which `enabledByTouchId` already names, or she beat the throw and nobody is charged. Before this the engine read that absence as a wild pitch — charging the pitcher for a batter's speed — which was the **last inference** in the WP/PB path. Error stays on the link rather than joining the enum, since it is already both a link and a charge and a third home could disagree with both. `FielderTouch.anchorEventId` becomes **`anchorEventId`**: since §15.6 a between-pitch entry anchors to the pitch, so the old name asserted a type the field no longer holds — and that misreading had already cost one silently un-charged error. Full history: `docs/spec-history.md`.
