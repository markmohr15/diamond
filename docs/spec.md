...
- **Play the fielder (drag + popup):** drag the fielder to where she made the play — wherever she is dropped is where the ball went. Her release point becomes the assumed `landing` (when no path was drawn) *and* the touch's own `location` (§4.2); she renders at that spot for the rest of the play (render-only — per-play repositioning is not §16.4 alignment data). A **what-happened popup** then disambiguates, options driven by the trajectory: in the air — *caught / dropped / missed it / picked it up*; on the ground — *fielded / booted / deflected / missed it*, where **deflected** is the ball that hit her and caromed away with no play to be made: a physical fact §13.2 never charges, and the ball stays loose. **"Missed it" records no touch at all**: she ran there and never touched the ball, which is officially nothing (§13 — no touch, no chargeable anything) beyond where the ball went. A **tap** on a fielder is the zero-distance drag: she played it where she stands.
